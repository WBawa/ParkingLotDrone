        ...
        DJISDKManager.getInstance().registerApp(getApplication(), object: DJISDKManager.SDKManagerCallback {
            override fun onRegister(error: DJIError?) {
                if (error == DJISDKError.REGISTRATION_SUCCESS) {
                    ...
                    Toast.makeText(getApplication(), "OnRegister: Registration Successful", Toast.LENGTH_SHORT).show()
                } else {
                    Log.i(ConnectionActivity.TAG, "onRegister: Registration Failed - ${error?.description}")
                    Toast.makeText(getApplication(), "onRegister: Registration Failed - ${error?.description}", Toast.LENGTH_SHORT).show()
                }
            }
